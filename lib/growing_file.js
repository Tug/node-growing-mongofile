 ...
 */
GrowingFile.open = function(mongodb, db, filename, options, callback) {
  var file = new this();
...
GrowingFile.createGridStore = function(mongodb, db, filename, metadata, callback) {
          ...
          self._gs.collection(function(err, fileCollection) {
            fileCollection.findOne({'_id': self._gs.fileId}, function(err, file) {
              ...
              if(!err && file && file.length == self.filesize) {
                self._gs._nthChunk(chunkNumber, callback);
              } else {
                callback(null, null);
              }
            });
